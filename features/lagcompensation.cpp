...
{
	// Get true latency
	INetChannelInfo *nci = (INetChannelInfo*)g_EngineClient->GetNetChannelInfo();
	if (nci)
	{
	...
	for (int s = 1; s <= g_GlobalVars->maxClients; ++s)
	{
		...
		if (m_PlayerRecords[s].m_vecPreviousOrigin.Length() > 0)
		{
			...
			if (delta.Length2DSqr() > 64)
			{
				...
				m_PlayerRecords[s].m_vecPreviousAngles = pEntity->GetAbsAngles();
		...
		m_PlayerRecords[s].m_vecAngles = pEntity->GetAbsAngles();
