...
bool C_AutoWall::IsBreakableEntity(C_BasePlayer* pBaseEntity)
{
	typedef bool(__thiscall* isBreakbaleEntityFn)(C_BasePlayer*);
	static isBreakbaleEntityFn IsBreakableEntityFn = (isBreakbaleEntityFn)Utils::PatternScan(GetModuleHandle(L"client.dll"), "55 8B EC 51 56 8B F1 85 F6 74 68");

	if (IsBreakableEntityFn)
	{
		// 0x280 = m_takedamage

		auto backupval = *reinterpret_cast<int*>((uint32_t)pBaseEntity + 0x280);
		auto className = pBaseEntity->GetClientClass()->m_pNetworkName;

		if (pBaseEntity != g_EntityList->GetClientEntity(0))
		{
			// CFuncBrush:
			...
			{
				*reinterpret_cast<int*>((uint32_t)pBaseEntity + 0x280) = 2;
			}
		}

		bool retn = IsBreakableEntityFn(pBaseEntity);

		*reinterpret_cast<int*>((uint32_t)pBaseEntity + 0x280) = backupval;
...
//	MessageBox(NULL, "RAD2DEG", "Maciej pFadddWare", NULL);
	...
	qAngles.x = RAD2DEG(atan(vDelta.z / fRoot));
//	MessageBox(NULL, "RAD2DEG2", "Maciej pFadddWare", NULL);
	qAngles.y = RAD2DEG(atan(vDelta.y / vDelta.x));
