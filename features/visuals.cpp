...
#include "../features/grenade_pred.hpp"
...
void Visuals::Player::RenderBox() {
	...
	g_VGuiSurface->DrawSetColor(ctx.clr);
...
/*	wchar_t buf[128];
	...
	if (MultiByteToWideChar(CP_UTF8, 0, info.szName, -1, buf, 128) > 0) {
		...
		g_VGuiSurface->DrawSetTextColor(ctx.clr);
		g_VGuiSurface->DrawSetTextPos(ctx.feet_pos.x - tw / 2, ctx.head_pos.y - th);
...
void Visuals::Player::RenderHealth()
{
	...
	auto height = box_h - (((box_h * hp) / 100));

	// calculate hp bar color.
	int red = min((510 * (100 - hp)) / 100, 255);
	int green = min((510 * hp) / 100, 255);
	...
	Color col_black = Color(0, 0, 0, (int)(255.f * ctx.pl->EntIndex()));
	g_VGuiSurface->DrawSetColor(col_black);
	g_VGuiSurface->DrawFilledRect(x, y, x + w, y + h);
	g_VGuiSurface->DrawSetColor(Color((250 - hp * int(2.55f)), (hp * int(2.55f)), 0, (int)(255.f * ctx.pl->EntIndex()))); //255
...
void Visuals::ThirdPerson() {
	...
	if (g_Options.misc_thirdperson && g_LocalPlayer->IsAlive())
	{
		...
		Vector desiredCamOffset = Vector(cos(DEG2RAD(view->y)) * dist,
			sin(DEG2RAD(view->y)) * dist,
			sin(DEG2RAD(-view->x)) * dist
		);
